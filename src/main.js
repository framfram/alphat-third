...
const    myBot    = [ ' u5ace79eb21357bceec8014f5e5194e9b']
const  banList  = []; // รายการที่ถูกแบน
...
\n";
        ...
        ถ้า ( txt      ===     'จ้ะ' && stateStatus this = = 1 & & isAdminOrBot (ต่อจาก From_) && seq. toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        } else         if ( txt         ===         'จ้ะ'     &&     isAdminOrBot ( seq . from_ ) &&       seq . toType       ==       2 ) { this . _sendMessage ( seq , "ไม่ได้รับสิทธิ์!" );
